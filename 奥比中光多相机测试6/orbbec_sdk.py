...
class OrbbecCameraSDK:
    ...
    def _define_function_prototypes(self):
        ...
        try:
            ...
            self.lib.oniStreamReadFrame.argtypes = [
                ...
                ctypes.c_int  # 超时时间（毫秒）- 这是缺失的关键参数！
            ]
    ...
    def read_frame(self, stream_handle: Optional[ctypes.c_void_p] = None,
        ...
        if not self.lib or not stream_handle:
            print("[SDK_DEBUG] 错误：lib或stream_handle无效")
            return None

        # 添加详细调试信息
        import time
        start_time = time.time()
        print(f"[SDK_DEBUG][{time.strftime('%H:%M:%S')}] read_frame开始, timeout={timeout}ms")

        # 检查流句柄值
        print(f"[SDK_DEBUG] 流句柄值: {stream_handle.value}")

        frame_ptr = ctypes.POINTER(self.OniFrame)()

        # 关键修复：传递timeout参数
        status = self.lib.oniStreamReadFrame(stream_handle, ctypes.byref(frame_ptr), timeout)
        elapsed = time.time() - start_time

        print(f"[SDK_DEBUG] oniStreamReadFrame返回, 状态码={status}, 耗时={elapsed:.2f}秒")

        if status != self.ONI_STATUS_OK:
            if status == self.ONI_STATUS_TIME_OUT:
                print(f"[SDK_DEBUG] 读取帧超时 (耗时{elapsed:.2f}秒)")
            else:
                print(f"[SDK_DEBUG] 读取帧失败，错误码: {status}")
            return None

        frame = frame_ptr.contents
        print(f"[SDK_DEBUG] 帧信息: 索引={frame.frameIndex}, 尺寸={frame.width}x{frame.height}")
    ...
    def capture_depth_frame(self, timeout: int = 1000, camera_id: str = "unknown") -> Optional[
        ...
        """
        print(f"[SDK_DEBUG] 相机{camera_id}开始capture_depth_frame, timeout={timeout}ms")

        frame_info = self.read_frame(self.depth_stream_handle, timeout)
        if not frame_info:
            print(f"[SDK_DEBUG] 相机{camera_id}读取失败")
            return None

        depth_data = frame_info['data']
        print(f"[SDK_DEBUG] 相机{camera_id}读取成功, 数据尺寸={depth_data.shape}")
