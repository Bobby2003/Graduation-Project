...
class ModuleLogger:
    ...
    def tracking_state(self, message: str, frame_id=None, force: bool = False):
        if self._should_print("tracking_state", frame_id=frame_id, force=force):
            self._print("TRACKING", message)

    def relocalization(self, message: str, frame_id=None, force: bool = False):
        if self._should_print("relocalization", frame_id=frame_id, force=force):
            self._print("RELOCALIZE", message)

    def mapping_state(self, message: str, frame_id=None, force: bool = False):
        if self._should_print("mapping_state", frame_id=frame_id, force=force):
            self._print("MAPPING", message)

    def pose(self, message: str, frame_id=None, force: bool = False):
        if self._should_print("pose", frame_id=frame_id, force=force):
            self._print("POSE", message)
