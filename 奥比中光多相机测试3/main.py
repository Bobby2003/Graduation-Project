...
from typing import Optional, List, Dict, Tuple  # 添加类型导入
...
def check_dependencies():
    ...
    try:
        ...
        print("⚠️ OpenNI2 (深度相机功能将受限)")
        print("   安装: pip install primesense")
        return False
...
def reconstruct_3d_model(captured_data: List[Dict[int, o3d.geometry.PointCloud]],
                         output_dir: str) -> Optional[o3d.geometry.PointCloud]:
    """3D重建模型
    Args:
        captured_data: 采集的点云数据列表，每个元素是相机ID到点云的映射字典
        output_dir: 输出目录路径

    Returns:
        重建后的点云对象，如果重建失败则返回None
    """
    ...
    try:
        ...
        # 尝试泊松表面重建（可选）
        reconstruction_config = MultiCameraConfig.RECONSTRUCTION_CONFIG
        if reconstruction_config.get('enable_mesh_reconstruction', False):
            print("执行泊松表面重建...")
            ...
            try:
                ...
                # 保存网格
                mesh_file = os.path.join(output_dir, "reconstructed_mesh.ply")
                o3d.io.write_triangle_mesh(mesh_file, mesh)
                print(f"✅ 网格重建完成，已保存: {mesh_file}")

                # 可以同时返回网格和点云
                return filtered_pcd

            except Exception as mesh_error:
                print(f"⚠️ 网格重建失败: {mesh_error}")
                print("继续使用点云...")
                return filtered_pcd
