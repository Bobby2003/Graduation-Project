...
def main():
    ...
    try:
        ...
        print(f"初始化失败: {e}")
        print("请确认已正确设置'OPENNI2_REDIST'环境变量。")
        return
    ...
    try:
        ...
        print(f"打开设备失败: {e}")
        print("请检查：1. 相机是否连接；2. 驱动是否安装；3. 是否被其他程序占用。")
        openni2.unload()
        return
    ...
    fx = 475.0  # 焦距 x
    fy = 475.0  # 焦距 y
    cx = 640 / 2  # 光学中心 x (基于640x480分辨率)
    cy = 480 / 2  # 光学中心 y

    # 点云累积与显示参数
    accumulated_pcd = o3d.geometry.PointCloud()  # 全局累积点云
    recent_frame_points = []  # 缓存最近几帧的点云数据
    recent_frame_colors = []  # 缓存最近几帧的颜色数据
    max_cache_frames = 5  # 最大缓存帧数 (平衡实时性与稳定性)
    ...
    vis.create_window(window_name="实时3D点云", width=640, height=480)
    # 设置渲染选项
    ...
    print("✅ Open3D可视化窗口已就绪")

    # 性能统计
    frame_count = 0
    start_time = time.time()
    last_3d_update_time = time.time()
    update_3d_interval = 0.5  # 更新3D显示的间隔(秒)，避免过于频繁刷新
    ...
    try:
        while True:
            ...
            # --- 4.2 为2D显示准备深度图 ---
            depth_valid = depth_array[depth_array > 0]
            if depth_valid.size > 0:
                max_depth_val = depth_valid.max()
            else:
                max_depth_val = 1
            depth_display = np.uint8((depth_array.astype(float) * 255.0 / max_depth_val))
            depth_display_color = cv2.applyColorMap(depth_display, cv2.COLORMAP_JET)

            # 在2D深度图上叠加信息
            fps_text = f"Frame: {frame_count}"
            cv2.putText(depth_display_color, fps_text, (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            points_text = f"Points: {len(accumulated_pcd.points)}"
            cv2.putText(depth_display_color, points_text, (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

            # --- 4.3 将深度图转换为3D点云 (关键步骤) ---
            # 毫米转米，并应用深度过滤
            depth_meters = depth_array.astype(np.float32) * 0.001  # 转换到米
            valid_mask = (depth_meters > min_depth) & (depth_meters < max_depth) & (depth_array > 0)
            ...
            if np.any(valid_mask):
                ...
                # 为点云着色 (基于深度图的伪彩色)
                depth_normalized = (z_valid - min_depth) / (max_depth - min_depth)
                depth_normalized = np.clip(depth_normalized, 0, 1)
                # 使用JET色彩映射：从蓝(近)到红(远)
                colors_jet = cv2.applyColorMap((depth_normalized * 255).astype(np.uint8).reshape(-1, 1, 1), cv2.COLORMAP_JET)
                colors_rgb = colors_jet.reshape(-1, 3)[:, [2, 1, 0]] / 255.0  # 转换BGR到RGB并归一化

                # 缓存当前帧的点云
                recent_frame_points.append(points_3d)
                recent_frame_colors.append(colors_rgb)

                # 限制缓存大小
                if len(recent_frame_points) > max_cache_frames:
                    recent_frame_points.pop(0)
                    recent_frame_colors.pop(0)

            # --- 4.4 定期更新全局点云并刷新3D显示 ---
            current_time = time.time()
            if current_time - last_3d_update_time > update_3d_interval and recent_frame_points:
                # 合并最近缓存的所有点云
                all_points = np.vstack(recent_frame_points)
                all_colors = np.vstack(recent_frame_colors)
                ...
                if len(all_points) > 10000:
                    accumulated_pcd = accumulated_pcd.voxel_down_sample(voxel_size=0.01)

                # 更新Open3D显示
                vis.clear_geometries()
                vis.add_geometry(coordinate_frame)
                vis.add_geometry(accumulated_pcd)
                last_3d_update_time = current_time
            ...
            # 显示2D深度图
            cv2.imshow('深度图 (按Q退出，S保存，C清除)', depth_display_color)
            ...
            if key == ord('q') or key == 27:  # 'q' 或 ESC
                ...
                if len(accumulated_pcd.points) > 0:
                    timestamp = time.strftime("%Y%m%d_%H%M%S")
                    filename = f"pointcloud_{timestamp}.ply"
                    o3d.io.write_point_cloud(filename, accumulated_pcd)
                    print(f"✅ 点云已保存: {filename}")
                else:
                    print("⚠️  当前无点云可保存")
            elif key == ord('c'):  # 清除点云
                accumulated_pcd.clear()
                recent_frame_points.clear()
                recent_frame_colors.clear()
                vis.clear_geometries()
                vis.add_geometry(coordinate_frame)
                print("🗑️  已清除所有累积点云")
        ...
        if duration > 0:
            print(f"运行统计: 共处理 {frame_count} 帧, 平均 {frame_count/duration:.1f} FPS")
